...
class CameraXSource(
  ...
  override fun create(width: Int, height: Int, fps: Int, rotation: Int): Boolean {
    ...
    override fun start(surfaceTexture: SurfaceTexture) {
        val facing =
            if (facing == CameraSelector.LENS_FACING_BACK) CameraHelper.Facing.BACK else CameraHelper.Facing.FRONT
        val optimalResolution = getOptimalResolution(Size(width, height), getCameraResolutions(facing).toTypedArray())
    ...
    fun switchCamera() {
        surfaceTexture?.let {
            stop()
            facing =
                if (facing == CameraSelector.LENS_FACING_BACK) CameraSelector.LENS_FACING_FRONT else CameraSelector.LENS_FACING_BACK
            start(it)
        }
    }
  ...
  private fun getMeteringFlag(): Int {
    ...
    fun takePicture(path: String, name: String, callBack: ImageCapture.OnImageSavedCallback) {
        val file = File(path)
        ...
        val outputOptions = ImageCapture.OutputFileOptions.Builder(File(file, name)).build()
    ...
    object : ImageCapture.OnImageSavedCallback {
                override fun onImageSaved(outputFileResults: ImageCapture.OutputFileResults) {
                    ...
                    Log.d("Camerasdfs", "Image saved to $savedUri")
                }
